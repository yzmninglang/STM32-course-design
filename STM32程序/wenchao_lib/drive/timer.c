 ...
 * @defualt[in]   TIM_IT		 常用中断类型     
 * @param[in]     Tim_arr		 抢占优先级   值：0,1,2,3
 * @param[in]     Tim_psc   使能/失能定时器，可选值ENABLE，DISABLE
 */
u8 TIM_us_Init_org(TIM_TypeDef* TIMx, u32 Tim_arr, u16 Tim_psc)
{
	...
	if( Tim_arr < 65535 && Tim_arr > 0 && Tim_psc < 10000 && Tim_psc > 0 ){
		arr = --Tim_arr;
		psc = --Tim_psc;
	}else{
		return 0;
 ...
 */
	void GPIO_PWM_Init(TIM_TypeDef* TIMx, u8 PWM_CHx)
	{
		if(TIMx == TIM1)
		{ switch(PWM_CHx)
			 {case PWM_CH1: GPIO_QuickInit(HW_GPIOA, GPIO_Pin_8, GPIO_Mode_AF_PP);  break;
				case PWM_CH2: GPIO_QuickInit(HW_GPIOA, GPIO_Pin_9, GPIO_Mode_AF_PP); break;
				case PWM_CH3: GPIO_QuickInit(HW_GPIOA, GPIO_Pin_10, GPIO_Mode_AF_PP); break;
				case PWM_CH4: GPIO_QuickInit(HW_GPIOA, GPIO_Pin_11, GPIO_Mode_AF_PP); break;
				default : break; }}
		else if(TIMx == TIM2)
		{  switch(PWM_CHx)
...
{ 	    	  	     
	  if(TIMER_CallBackTable[HW_TIMER1] != NULL)
		{
			TIMER_CallBackTable[HW_TIMER1]();
		}
		else
		{
